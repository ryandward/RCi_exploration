...
rpoE_MPRASet <- MPRASet(
	RNA = rpoE_data %>% filter(nucleotide == "RNA") %>% select(-promoter, -nucleotide),
	DNA = rpoE_data %>% filter(nucleotide == "DNA") %>% select(-promoter, -nucleotide),
	eid = rpoE_data %>% filter(nucleotide == "RNA") %>% pull(promoter) %>% as.character 
)
