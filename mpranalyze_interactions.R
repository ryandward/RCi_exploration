...
rci_mpranalyze_intx_comp <- analyzeComparative(
	rci_mpranalyze,
	dnaDesign = ~ timing,
	rnaDesign = ~ timing + promoter + timing:promoter,
	reducedDesign = ~ timing,
	fit.se = TRUE)

# Save comparative results
rci_mpranalyze_intx_comp %>% saveRDS("processed_results/rci_mpranalyze_intx_comp")

rci_logFC <- rci_mpranalyze_intx_comp@modelFits$r.coef %>% data.table(keep.rownames = "spacer") %>% set_colnames(c("spacer", "disp", colnames(rci_mpranalyze_intx_comp@designs@rnaFull)))

rci_se <- rci_mpranalyze_intx_comp@modelFits$r.se  %>% data.table(keep.rownames = "spacer") %>% set_colnames(c("spacer", "disp", colnames(rci_mpranalyze_intx_comp@designs@rnaFull)))
...
rci_results <- rci_results %>% 
	filter(logFC != 0) %>%
	inner_join(rci_mpranalyze_intx_comp@modelFits$r.df %>% data.frame(r.df = .) %>% data.table(keep.rownames = "spacer")) %>%
...
rci_results %>% inner_join(rci_mpranalyze_intx_comp@modelFits$r.df %>% data.frame(r.df = .) %>% data.table(keep.rownames = "spacer"))
...
guides %>% inner_join(rci_mpranalyze_intx_comp@modelFits$r.coef %>% data.table(keep.rownames = "spacer") %>% set_colnames(c("spacer", "disp", colnames(rci_mpranalyze_intx_comp@designs@rnaFull)))) %>% melt(id.vars = c("gene", "type", "locus_tag", "offset", "spacer"), variable.name = "predictor", value.name = "logFC")  %>% inner_join(testLrt(rci_mpranalyze_intx_comp) %>% data.table(keep.rownames = "spacer") %>% arrange(desc(statistic))) %>% View