...
lane_spread_matrix_RNA[is.na(lane_spread_matrix_RNA)] <- 0

gc()

lane_spread_matrix_RNA_norm <- normalize_data(lane_spread_matrix_RNA)


##########################################################################################

k_choice <- choose_k(lane_spread_matrix_RNA_norm, noise_start = 25)
...
# lane_spacers_norm_complete_RNA <- alra(lane_spread_matrix_RNA_norm, k = k_choice$k)[[3]]
lane_spacers_norm_complete_RNA <- alra(lane_spread_matrix_RNA_norm, k = 5)[[3]] %>% 
	exp() %>%  `-`(1) %>% round(3) %>%
	set_rownames(sample_identifiers$identifier) %>% 
	t %>% 
